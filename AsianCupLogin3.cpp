...
void MatchReservation::MatchDay() { cout << "원하는 날짜를 고르세요" << endl; }
...
void MatchReservation::ReservationPage() {
  // system("clear");
  cout << "------------------------" << endl;
...
void CreateAccount::SignUpPage() {
  ...
  if (c == "1") // 1. 로그인하기 눌렀을 때
  {
    ...
    getline(cin, line);
    // while (a.PwDu(line) == 0) {
    //   cout << "입력하신 비밀번호와 ID가 일치하지 않습니다. 다시
    //   입력하셈 : "; getline(cin, line);
    // }
    cout << "로그인 되셨습니다!" << endl;
    d.ReservationPage();
  } else if (c == "2") // 2. ID찾기 눌렀을 때
  {
    ...
    getline(cin, line);
    cout << "찾아낸 ID는 " << IdFind(line) << " 입니다." << endl;
    SignUpPage();
  ...
  } else if (c == "3") // 3. 비밀번호 찾기 눌렀을 때
  {
    ...
    cout << "전화번호 : ";
    getline(cin, line);
    cout << "찾아낸 비밀번호는 " << PwFind(line) << " 입니다." << endl;
    SignUpPage();
...
string
CreateAccount::IdFind(string phonenumber) { //전화번호를 이용해 ID를 찾는 함수
  string s = "그런 ID는 읎다.";
...
string
CreateAccount::PwFind(string phonenumber) { //전화번호를 이용해 ID를 찾는 함수
...
int CreateAccount::PwSet(string pw) //비밀번호를 처음 설정할 때 기준에
                                    //맞추어 해야한다(5글자 이상).
{
  if (pw.size() < 5)
    return 0;
...
int CreateAccount::MemBerNumber() {
  ...
  MemberNumber.push_back(
      Mnumber); //형 변환해서 회원정보 vector 마지막 부분에 넣기
  return Mnumber;
}
